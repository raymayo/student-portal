...
export const assignTeacherToSchedules = async (req, res) => {
  try {
    ...
    if (!teacherId || !Array.isArray(scheduleIds) || scheduleIds.length === 0) {
      return res
        .status(400)
        .json({ error: "Teacher ID and schedule IDs are required" });
    }
    ...
    teacher.teachingSchedules = [
      ...new Set([...teacher.teachingSchedules, ...scheduleIds]),
    ];
    await teacher.save();

    res.json({
      message: "Teacher assigned successfully to multiple schedules",
      schedules,
      teacher,
    });
  } catch (error) {
...
export const assignSchedulesToStudent = async (req, res) => {
  try {
    ...
    const existingScheduleIds = student.currentSubjects.map((id) =>
      id.toString(),
    ); // Convert to string for comparison
    const newSchedules = scheduleIds.filter(
      (id) => !existingScheduleIds.includes(id.toString()),
    ); // Only add new schedules

    if (newSchedules.length === 0) {
      return res
        .status(400)
        .json({ message: "Selected schedules are already assigned." });
    }

    // **Assign only new schedules**
    student.currentSubjects.push(
      ...newSchedules.map((id) => new mongoose.Types.ObjectId(id)),
    );
    ...
    res
      .status(200)
      .json({ message: "Schedules assigned successfully!", student });
  } catch (error) {
