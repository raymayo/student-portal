...
export const autoAssignStudentsToSet = async (req, res) => {
  try {
    ...
    for (const student of students) {
      ...
      if (Object.keys(setCounts).length > 0) {
        ...
        for (const [setId, count] of Object.entries(setCounts)) {
          ...
          const setYearLevel = Math.max(
            ...set.schedules.map((sch) => sch.yearLevel),
            0,
          );

          if (
            count > maxCount ||
            (count === maxCount && setYearLevel > highestYearLevel)
          ) {
            bestSetId = setId;
            maxCount = count;
    ...
    res
      .status(200)
      .json({ message: "Students auto-assigned to Sets successfully!" });
  } catch (error) {
...
export const manualAssignStudentToSet = async (req, res) => {
  try {
    ...
    if (student.set) {
      ...
      if (prevSet) {
        prevSet.students = prevSet.students.filter(
    ...
    res
      .status(200)
      .json({ message: "Student manually assigned to Set.", student });
  } catch (error) {
